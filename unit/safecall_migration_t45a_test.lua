...
---------------------------------------------------------------------------
-- File: QUI_UnitFrames/unitframes/castbar.lua + core/cast_engine.lua
-- QUI-PTR architecture note: the SetTimerDuration DurationObject forwards
-- live in CastEngine.ApplyTimerDriven (core/cast_engine.lua, shared with
-- nameplates), not inline in castbar.lua as on the qui5-alpha line. The
-- font fallback stays in castbar.lua (1 site, best-effort-style); the
-- engine bind is 2 sites (primary + direction-less retry, sink-forward).
---------------------------------------------------------------------------
local cb = readAll("QUI_UnitFrames/unitframes/castbar.lua")
local ce = readAll("core/cast_engine.lua")
...
check("castbar.lua: 1x SafeCall( in this file (font fallback; timer binds live in cast_engine)",
    countOf(cb, "ns.SafeCall(") + countOf(cb, "ns.SafeCallMethod(") == 1)
check("cast_engine.lua: 2x ns.SafeCallMethod(\"sink-forward\", statusBar, \"SetTimerDuration\", ...)",
    countOf(ce, 'ns.SafeCallMethod("sink-forward", statusBar, "SetTimerDuration"') == 2)
check("cast_engine.lua: NO bare pcall(statusBar.SetTimerDuration remains",
    ce:find("pcall(statusBar.SetTimerDuration", 1, true) == nil)
...
-- Task 45a2 (QUI-PTR shape): the engine bind keeps `ok` (feeds a plain
-- pcall-success fallback retry, NOT a secret truth-test) + the
-- direction-less retry.
check("cast_engine.lua (45a2): ApplyTimerDriven bind converted with retry",
    ce:find('local ok = ns.SafeCallMethod("sink-forward", statusBar, "SetTimerDuration", durationObj, 0, direction or 0)\n    if not ok then\n        -- Fallback: older signature without the direction parameter\n        ok = ns.SafeCallMethod("sink-forward", statusBar, "SetTimerDuration", durationObj)', 1, true) ~= nil)

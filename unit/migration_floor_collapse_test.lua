...
-- It re-enters the single gate. Upstream (DrewUI) additionally rebrands
-- legacy QUI profile keys here; QUI's keys ARE those names, so ApplyRebrand
-- is a no-op and the container must simply survive the re-entry untouched.
do
    local profile = {
        _schemaVersion = 59,
    ...
    M.RunOnProfile(profile)
    check("alpha stamp (59) re-entry: quiUnitFrames survives",
    ...
    check("alpha stamp (59) font sentinel rewritten",
